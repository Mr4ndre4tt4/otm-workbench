...
def test_bootstrap_qa_user_cli_uses_synthetic_defaults():
    database_path = Path("var") / f"cli-{uuid4()}.db"
    env = {
        **os.environ,
        "OTM_DATABASE_URL": f"sqlite:///{database_path}",
    ...
    assert result.returncode == 0, result.stderr
    assert "QA browser user ready: demo@example.test" in result.stdout
    assert "Admin privileges: true" in result.stdout
