...
6. Template Builder para gerar Excel. [INICIADO com endpoint build-workbook gerando XLSX sintético, Artifact client-safe e download escopado por template com hash]
...
16. Scenario packs funcionais Location Operational e Item Packaging. [INICIADO com QA backend, editor backend-owned, relacoes same-batch, erro detalhado de editor e handoff Location + Item Packaging -> Load Plan/Cutover]
```
...
26. Preview executavel local que materializa JSON alvo sintetico com proveniencia de campo. [ENTREGUE fatias backend OTM-131/OTM-132/OTM-136: direct scalar XML -> JSON, single loop XML -> JSON array e MOCK lookup escalar com field_provenance em 2026-05-25; joins e lookups em loop seguem metadata-only/backlog]
