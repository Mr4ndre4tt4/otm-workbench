...
| Developer Tools hub, Data Dictionary Explorer, FK Catalog Explorer, Schema Pack Diagnostics, and Environment Readiness | `GET /api/v1/platform/dev-tools/summary`, `GET /api/v1/platform/dev-tools/data-dictionary`, `GET /api/v1/platform/dev-tools/data-dictionary/tables/{table_name}`, `GET /api/v1/platform/dev-tools/fk-catalog?source_table=...`, `GET /api/v1/platform/dev-tools/schema-packs?otm_version=...&code=...`, `GET /api/v1/platform/dev-tools/environment-readiness` guarded by admin access and `dev_tools` feature flag | `useDevToolsSummary`, `useDevToolsDataDictionary`, `useDevToolsDataDictionaryTable`, `useDevToolsFkCatalog`, `useDevToolsSchemaPacks`, `useDevToolsEnvironmentReadiness`, `DeveloperToolsView`, `DeveloperToolsDataDictionaryView`, `DeveloperToolsDataDictionaryTableView`, `DeveloperToolsFkCatalogView`, `DeveloperToolsSchemaPacksView`, `DeveloperToolsEnvironmentReadinessView` | `test_dev_tools_summary.py`, `App.test.tsx`, Developer Tools browser QA | READY | Summary exposes active context, gates, available/disabled tool metadata, disabled reasons, counts, and recent run metadata only. Data Dictionary list/detail, FK Catalog, Schema Pack Diagnostics, and Environment Readiness responses are client-safe and do not expose local file paths, raw diagnostic payloads, connection details, or credentials; table detail, FK list, schema-root list, and readiness checks stay compact with visible summaries. |
