...
| Master Data / Data Factory | templates with available_actions, drafts, versions, publish, validate definition, workbook build, workbook editor contract/validate/batch creation, batches with available_actions, filter-only batch summary, relationship validation, map, output, CSV build/preview/export, artifacts/download | `masterData.ts`, `MasterDataView` | Master Data React + browser QA, master data/load plan intake tests | READY | Direct OTM import, richer audited spreadsheet editing, and deeper history analytics are future scope. |
