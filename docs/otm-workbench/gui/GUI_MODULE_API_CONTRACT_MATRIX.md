...
| Integration Mapping Studio | systems/endpoints, definitions, payload artifacts, schema parse/docs/nodes, mappings with backend-owned transform_config, loops, joins, lookups, validate with spec/preview readiness, preview, spec, artifacts/download | `integrationMapping.ts`, `IntegrationMappingStudioView` | Integration Mapping React + browser QA, integration backend tests | READY | OTM-130 added persisted transform config for controlled transforms; UI authoring and executable target JSON preview remain explicit future scope. |
