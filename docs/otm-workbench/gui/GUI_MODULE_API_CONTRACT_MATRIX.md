...
| Integration Mapping Studio | systems/endpoints, definitions, payload artifacts, schema parse/docs/nodes, mappings with backend-owned transform_config, loops, joins, lookups, validate with spec/preview readiness, direct scalar executable preview with provenance, metadata-only preview fallback, spec, artifacts/download | `integrationMapping.ts`, `IntegrationMappingStudioView` | Integration Mapping React + browser QA, integration backend tests | READY | OTM-130 added persisted transform config; OTM-131 added first executable preview slice. UI authoring, loop execution and join/lookup execution remain explicit future scope. |
