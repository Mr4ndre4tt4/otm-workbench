...
| Master Data / Data Factory | templates, drafts, versions, publish, validate definition, workbook build, batches, relationship validation, map, output, CSV build/preview/export, artifacts/download | `masterData.ts`, `MasterDataView` | Master Data React + browser QA, master data/load plan intake tests | READY | Direct OTM import, spreadsheet editing, and richer history are future scope. |
