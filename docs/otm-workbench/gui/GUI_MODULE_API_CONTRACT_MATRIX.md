...
| Master Data / Data Factory | templates with available_actions, drafts, versions, publish, validate definition, workbook build, workbook editor contract/validate/batch creation, batches with available_actions, filter-only batch summary, relationship validation, map, output, CSV build/preview/export, artifacts/download, guarded direct OTM import readiness/submit refusal | `masterData.ts`, `MasterDataView` | Master Data React + browser QA, master data/direct-import-guard/load-plan intake tests | READY | Real direct OTM import, richer audited spreadsheet editing, and deeper history analytics are future scope. |
...
| Integration Mapping Studio | systems/endpoints, definitions, payload artifacts, schema parse/docs/nodes, mappings, loops, joins, lookups, validate with spec/preview readiness, preview, spec, artifacts/download | `integrationMapping.ts`, `IntegrationMappingStudioView` | Integration Mapping React + browser QA, integration backend tests | READY | Current MVP0 journey complete; harden only with explicit new scope. |
