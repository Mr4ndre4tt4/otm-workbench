...
| Integration Mapping Studio | systems/endpoints, definitions, payload artifacts, schema parse/docs/nodes, mappings with backend-owned transform_config, loops, joins, lookups, validate with spec/preview readiness, direct scalar/single-loop/lookup executable preview with provenance, CONSTANT/DATE_FORMAT/CONCAT transform execution, metadata-only preview fallback, spec, artifacts/download | `integrationMapping.ts`, `IntegrationMappingStudioView` | Integration Mapping React + browser QA, integration backend tests | READY | OTM-130 added persisted transform config; OTM-131/132/136 added executable preview slices; OTM-133/134/135 execute CONSTANT, OTM_GLOGDATE -> ISO8601 DATE_FORMAT, and controlled CONCAT. UI authoring, nested/multi-hop loop execution and join execution remain explicit future scope. |
