...
| Master Data / Data Factory | templates, drafts, versions, publish, validate definition, workbook build, batches, filter-only batch summary, relationship validation, map, output, CSV build/preview/export, artifacts/download | `masterData.ts`, `MasterDataView` | Master Data React + browser QA, master data/load plan intake tests | READY | Direct OTM import, spreadsheet editing, and deeper history analytics are future scope. |
