...
| `/master-data` | OTM-114 | Follow the staged Data Factory workflow: select template, validate template, build workbook, upload synthetic workbook, validate relationships, map records, build output, inspect backend-owned output preview, build CSV, inspect CSV preview, export package, register exported package for Load Plan, filter durable batch history, leave route, and return with backend-owned template state visible. | DONE: `AppFunctionalMasterData.test.tsx` covers staged workflow navigation, template validation, workbook generation, FormData upload, invalid upload recovery, relationship validation, map, output preview, CSV preview, export, Load Plan package registration, backend batch history filters, bearer-auth API assertions, and route return-state. | DONE: `npm run qa:functional:master-data:browser` covers login, Data Factory navigation, explicit `REGIONS_BASIC` selection, template validation, workbook generation, synthetic XLSX upload, relationship validation, map, output preview, CSV preview, export package, Load Plan package registration, backend batch filters, route return to Output, durable batch/artifact visibility, console health, and HTTP health against local FastAPI + Vite. | DONE: `test_master_data_templates.py` covers template validation, workbook generation, upload parsing, relationship validation, mapping, output records, output record listing, CSV files, CSV preview listing, filtered/paginated batch listing, export package, artifacts, evidence, manifest behavior, and idempotent CSV/export retries. `test_load_plan_package_intake.py` covers Master Data package intake, idempotency, evidence, audit, and event creation. | Template state and durable batch/artifact rows remain visible after leaving and returning through backend list endpoints. | Spreadsheet editing, deeper Load Plan checklist handoff, direct OTM import, and richer batch history dimensions remain follow-up scope. |
...
/master-data:
  ...
  registration from exported batch, Coordinate Quality validate/batches/results/export.
