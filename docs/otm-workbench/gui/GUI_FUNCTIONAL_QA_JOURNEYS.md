...
| `/order-release-generator` | OTM-95, OTM-125 | Select template, author batch rows from template-guided fields, create batch, preview XML, generate XML artifact, download XML through the guarded backend artifact endpoint, verify guarded OTM submit state. | DONE: `AppFunctionalOrderReleaseGenerator.test.tsx` covers template selection, template-guided row editor, edited row payload, backend batch creation, XML preview, XML artifact generation, artifact list/download URL, guarded download call, submit guard, bearer-auth assertions, and route return-state through backend batch list. | DONE: `npm run qa:functional:order-release:browser` covers login, context setup, template selection, batch creation, XML preview, XML artifact generation, guarded artifact download, guarded submit, route return, console health, and HTTP health against local FastAPI + Vite. | DONE: OR generator backend tests cover batches, batch listing, preview, XML artifact, artifact list/download with hash verification and audit, cross-batch artifact rejection, submit guard, jobs, evidence, and client-safe synthetic data. | Recent batch state remains visible after leaving and returning through backend-owned `/batches` list. Preview/artifact/download/submit guard state is route-session scoped. | Reusable template authoring/versioning and invalid-batch recovery UX remain follow-up scope. |
...
/order-release-generator:
  templates, batches, preview-xml, generate-xml-artifact, artifact list/download, submit guard.
