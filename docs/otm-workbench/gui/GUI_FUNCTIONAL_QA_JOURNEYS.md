...
| `/integration-mapping` | OTM-96, OTM-152, OTM-153 | Follow the staged workflow: systems/endpoints, definition, payloads/schemas, mapping rules, definitions list. Create source/target payloads, parse schema documents, create mapping, alias-backed mapping, loop, semantic loop join, multi-hop join binding, and lookup rules from schema node selectors/drafts, reset mapping-rule drafts, validate, preview, download and inspect the executable preview artifact, generate spec, list generated artifacts, download the generated spec, switch to another definition with a clean definition workspace, leave and return with backend-owned definition still visible. | DONE: `AppFunctionalIntegrationMapping.test.tsx` covers staged navigation, system/endpoint creation, definition creation, manual payload/schema creation, schema node path picking for mapping/alias-backed mapping/loop/join/join-binding/lookup, mapping/alias/loop/join/join-binding/lookup payload/auth assertions, grouped review rows for join bindings and aliases, mapping-rule draft reset recovery with stale feedback cleared, validate, preview, spec generation, generated artifact listing/download, definition-switch recovery with stale feedback cleared and mapping schema drafts reset, and return-state check. | DONE: `npm run qa:functional:integration-mapping:browser` covers login, Integration Mapping navigation, staged workflow navigation, real system/endpoint/definition/manual payload/schema creation, schema node path picking, header mapping, loop mapping, semantic loop join, multi-hop join-binding, alias-backed header and `Entregas[]` mappings, loop-scoped lookup creation, mapping-rule draft reset recovery, validate, executable preview artifact download/content assertions, spec generation, artifact download, two-definition switch recovery with stale artifact feedback cleared, route return, console health, and HTTP health against local FastAPI + Vite. | DONE: integration mapping backend tests cover systems/endpoints, definitions, payloads, schema persistence/tree, mappings, alias validation, loops, joins, join bindings, mixed header + loop alias preview, lookups, validation, preview, spec, generated artifacts/download, audit, and synthetic E2E. | Created system/endpoint, definition, manual payload schemas, mapping, alias-backed mapping, loop, join, join binding, lookup, executable preview artifact, and generated spec metadata remain visible after leaving and returning; rule draft fields and definition-scoped feedback can return to a clean baseline without route reload. | Negative alias/path browser recovery remains in `OTM-153`; richer required-field semantics remain explicit hardening. |
