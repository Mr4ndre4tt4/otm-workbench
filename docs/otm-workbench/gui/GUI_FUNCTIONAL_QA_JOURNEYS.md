...
| `/order-release-generator` | OTM-95, OTM-125, OTM-146 | Create a reusable backend-owned template, create a new template version, select template/version, author batch rows from template-guided fields, reset rows from selected template defaults, create batch, inspect invalid row issues when present, correct rows, preview XML, generate XML artifact, download XML through the guarded backend artifact endpoint, verify guarded OTM submit state, switch templates and recover to a clean batch workspace. | DONE: `AppFunctionalOrderReleaseGenerator.test.tsx` covers backend-owned custom template creation from the Templates stage, backend-owned template version creation, template selection, template-guided row editor, reset-row recovery from template defaults, template-switch recovery with stale batch/preview/artifact/submit state cleared, template-switch row reset to the new template defaults, edited row payload, backend batch creation, invalid row issue display, invalid batch Preview/Artifact blocking, corrected-row recreate, XML preview, XML artifact generation, artifact list/download URL, guarded download call, submit guard, bearer-auth assertions, and route return-state through backend batch list. | DONE: `npm run qa:functional:order-release:browser` covers login, context setup, custom alternate template creation, template selection, row reset recovery, batch creation, XML preview, XML artifact generation, guarded artifact download, guarded submit, template-switch recovery, route return, console health, and HTTP health against local FastAPI + Vite. `npm run qa:chaos:browser` third slice covers switching templates after dirty row editing, add/remove row, valid batch creation, XML preview, and guarded submit feedback; the selected template switch clears active batch, stale preview, stale submit guard, stale extra row, previous-template-only fields, and previous row values. | DONE: OR generator backend tests cover template list/create/version validation, template version schema migration, batches, batch listing, preview, XML artifact, artifact list/download with hash verification and audit, cross-batch artifact rejection, submit guard, jobs, evidence, and client-safe synthetic data. | Recent batch state remains visible after leaving and returning through backend-owned `/batches` list. Created templates and versions are backend-owned and reloaded through `/templates`. Preview/artifact/download/submit guard state is route-session scoped and cleared when the selected backend template changes; batch row drafts reset to the selected template defaults. | Direct OTM submit remains guarded/out of MVP0. |
| `/integration-mapping` | OTM-96, OTM-146, OTM-152, OTM-153, OTM-154, OTM-159, OTM-160 | Follow the staged workflow: systems/endpoints, definition, payloads/schemas, mapping rules, definitions list. Create source/target payloads, parse schema documents, create mapping from backend-owned suggestions or schema node selectors, create a selected suggestion review set through the backend bulk contract, alias-backed mapping, loop, semantic loop join, multi-hop join binding, and lookup rules from schema node selectors/drafts, reset mapping-rule drafts, validate, preview, download and inspect the executable preview artifact, generate spec, list generated artifacts, download the generated spec, switch to another definition with a clean definition workspace, create an invalid alias/path mapping, observe backend blocker, remove the invalid mapping, recover with a valid mapping, create a required-target scenario definition, observe missing required targets, complete the checklist, leave and return with backend-owned definition still visible. | DONE: `AppFunctionalIntegrationMapping.test.tsx` covers staged navigation, system/endpoint creation, definition creation, manual payload/schema creation, official Catalog path selection for direct source mappings, target mappings, and loop source collections, backend-owned suggestion confidence/reason visibility before apply, backend-owned transform-type suggestion facet, local selected-suggestion review with source/target/transform/confidence/reason before apply, visible selected-suggestion bulk creation through the backend-owned `/mappings/bulk` contract, selected-suggestion clear recovery without dropping loaded suggestions, selected-suggestion cleanup after mapping creation, schema node path picking for mapping/alias-backed mapping/loop/join/join-binding/lookup, mapping/alias/loop/join/join-binding/lookup payload/auth assertions, backend-owned mapping removal/recreation, scenario-pack required target checklist rendering, grouped review rows for join bindings and aliases, mapping-rule draft reset recovery with stale feedback cleared, suggestion-list clearing after reset/definition switch, validate, preview, spec generation, generated artifact listing/download, definition-switch recovery with stale feedback cleared and mapping schema drafts reset, and return-state check. Backend-owned suggestion and bulk mapping contracts are covered by focused pytest for ownership validation, exact scoring, ambiguous scoring, first OTM-context scoring, all-or-nothing bulk create, and partial-create rejection. | DONE: `npm run qa:functional:integration-mapping:browser` covers login, Integration Mapping navigation, staged workflow navigation, real system/endpoint/definition/manual payload/schema creation, backend-owned mapping suggestion load/apply, non-trivial `StopSequence -> deliveries[].sequence` suggestion visibility, schema node path picking, header mapping, loop mapping, semantic loop join, multi-hop join-binding, alias-backed header and `Entregas[]` mappings, loop-scoped lookup creation, mapping-rule draft reset recovery, validate, executable preview artifact download/content assertions, blocked alias/path validation, blocked preview without stale success, backend-owned mapping removal, correction without route reload, required target scenario-pack missing/covered checklist, spec generation, artifact download, two-definition switch recovery with stale artifact feedback cleared, route return, console health, and HTTP health against local FastAPI + Vite. `npm run qa:chaos:browser` first slice covers dirty mapping draft plus stale backend-owned suggestions when switching definitions out of order. | DONE: integration mapping backend tests cover systems/endpoints, definitions, payloads, schema persistence/tree, mappings, backend-owned mapping suggestions with schema ownership validation, exact/context/ambiguous scoring, all-or-nothing bulk create, mapping delete, alias validation, required target scenario-pack summaries, loops, joins, join bindings, mixed header + loop alias preview, lookups, validation, preview, spec, generated artifacts/download, audit, and synthetic E2E. | Created system/endpoint, definition, manual payload schemas, mapping, bulk-created selected suggestion mappings, alias-backed mapping, loop, join, join binding, lookup, executable preview artifact, required target checklist, and generated spec metadata remain visible after leaving and returning; rule draft fields, definition-scoped feedback, and suggestion rows can return to a clean baseline without route reload; invalid mapping can be removed and replaced without reload. | Broader Catalog/Data Dictionary synonym library and richer required-field semantics remain explicit hardening. |
| `/dev-tools`, `/dev-tools/data-dictionary`, `/dev-tools/data-dictionary/tables/:tableName`, `/dev-tools/fk-catalog`, `/dev-tools/schema-packs`, and `/dev-tools/environment-readiness` | Developer Tools | Open Developer Tools as an authorized technical route, confirm it is a dedicated guarded hub rather than the generic placeholder, click the backend-owned Data Dictionary, FK Catalog, Schema Pack, and Environment Readiness tool routes, inspect filtered technical metadata, open a table detail route, inspect compact column/FK/schema-root/environment readiness metadata, and return through visible route links. | DONE: `App.test.tsx` covers `/dev-tools` with backend navigation item `dev_tools`, verifies `Technical Diagnostics Hub`, backend-owned summary rows, disabled tool reason, recent run metadata, safe-output copy, explicit tool links, absence of the generic placeholder panel, dedicated `/dev-tools/data-dictionary?query=rate_geo`, direct `/dev-tools/data-dictionary/tables/RATE_GEO_COST`, `/dev-tools/fk-catalog?source_table=RATE_GEO_COST`, `/dev-tools/schema-packs?otm_version=26A`, and `/dev-tools/environment-readiness` rendering with source contracts and Back links. | DONE: `npm run qa:functional:developer-tools:browser` enables the backend `dev_tools` feature flag, seeds synthetic context plus a safe `dev_tools` `DEMO_ECHO` run, opens `/dev-tools`, verifies backend summary tools/runs, clicks the Data Dictionary route, verifies filtered `rate_geo` results, opens `RATE_GEO_COST` detail, opens FK Catalog, seeds and indexes a synthetic schema pack through Catalog Core APIs, opens Schema Pack Diagnostics, opens Environment Readiness, verifies active environment/domain readiness, checks console/HTTP health, and captures `output/gui-qa/developer-tools/01-developer-tools-guarded-hub.png` through `output/gui-qa/developer-tools/10-environment-readiness-mobile.png`. | DONE: `test_dev_tools_summary.py` covers authentication, feature-flag guard, backend-owned summary tools/disabled reasons/counts/guards/active context, recent run metadata, no raw diagnostic payload leakage, Data Dictionary list guard/list payload, table detail payload without local file paths, FK Catalog feature-flag guard, source-table not-found behavior, FK Catalog payload without local file paths, Schema Pack Diagnostics feature-flag guard, code-filtered schema pack list, root preview metadata, no source path leakage, Environment Readiness feature-flag guard, active-context checks, safe environment scope, and no connection/password leakage. | Recent run metadata is backend-owned and scoped to active project when selected; Data Dictionary query, FK Catalog source table, and Schema Pack filters are URL-owned; table detail Back returns to the filtered explorer; FK parent links open Data Dictionary detail routes; Schema Pack Diagnostics leaves deep path browsing to Catalog Core; Environment Readiness reflects the active context without exposing connection details. | Diagnostic-run detail, FK trace actions, and richer Data Dictionary/FK/schema filtering remain explicit next scope. |
...
/integration-mapping:
  ...
  tree/documents/nodes, backend-owned mapping suggestions, mappings, loops,
  joins, lookups, validate, preview, generate-spec, generated artifacts,
  guarded artifact download.

/dev-tools:
  summary, Data Dictionary table list, Data Dictionary table detail,
  FK Catalog relationship list, Schema Pack Diagnostics list.
...
npm run qa:functional:developer-tools:browser
...
5. Integration Mapping authoring journey: OTM-96.
   Current browser journey also covers OTM-152 through OTM-157: executable
