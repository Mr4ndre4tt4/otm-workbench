...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `/` and `/home` | OTM-101 | Login, load cockpit, apply active context, refresh readiness, update preferences, navigate away and return. | DONE: `AppFunctionalShell.test.tsx` plus `PreferenceControls.test.tsx` overlap guard for rapid preference clicks. | DONE: `npm run qa:functional:shell:browser`. | DONE: `test_gui_shell_backend_contracts.py` covers login/session, backend navigation icon/label metadata, active context, persisted preferences, cockpit summary, capabilities, and unauthenticated shell read guards. | Context and preferences remain visible after leaving and returning. | None for the current shell foundation journey; future preference work should be explicit new scope. |
...
| `/catalog` | OTM-90 | Search table/column/reference, validate input, inspect macro object dependencies and load plan. | DONE: `AppFunctionalCatalog.test.tsx` covers macro detail/tables/load plan plus table, column, and reference validation API calls/results, then reset recovery back to default synthetic validation fields with stale results cleared. | DONE: `npm run qa:functional:catalog:browser` covers Catalog navigation, explicit RATE_RECORD selection, table/load-plan inspection, table validation, column validation, reference validation, validation reset recovery, route return, console health, and HTTP health against local FastAPI + Vite. | DONE: `test_catalog_core.py`. | Last selected macro/table can be reselected and still matches backend response; validation form can return to a known baseline without route reload. | Reference option browser remains future enhancement; current validation journey is covered. |
| `/master-data` | OTM-114 | Follow the staged Data Factory workflow: select template, validate template, build workbook, upload synthetic workbook, validate relationships, map records, build output, inspect backend-owned output preview, build CSV, inspect CSV preview, export package, register exported package for Load Plan, create cutover checklist, generate checklist readiness, inspect readiness blockers, filter durable batch history, leave route, and return with backend-owned template state visible. | DONE: `AppFunctionalMasterData.test.tsx` covers staged workflow navigation, template validation, workbook generation, FormData upload, invalid upload recovery, relationship validation, map, output preview, CSV preview, export, Load Plan package registration, cutover checklist creation/readiness, readiness blocker display, backend batch history filters including file name and minimum rows, backend batch history metrics, durable batch inspection/recovery, active-row `aria-current`, bearer-auth API assertions, and route return-state. | DONE: `npm run qa:functional:master-data:browser` covers login, Data Factory navigation, explicit `REGIONS_BASIC` selection, template validation, workbook generation, synthetic XLSX upload, relationship validation, map, output preview, CSV preview, export package, Load Plan package registration, cutover checklist creation/readiness with backend blocker display, backend batch filters and reset recovery, active durable batch row `aria-current`, route return to Output, durable batch/artifact visibility, console health, and HTTP health against local FastAPI + Vite. | DONE: `test_master_data_templates.py` covers template validation, workbook generation, upload parsing, relationship validation, mapping, output records, output record listing, CSV files, CSV preview listing, filtered/paginated batch listing with file-name and minimum-row filters, filtered batch-history summary metrics, export package, artifacts, evidence, manifest behavior, and idempotent CSV/export retries. `test_load_plan_package_intake.py` covers Master Data package intake, idempotency, evidence, audit, and event creation; Load Plan checklist tests cover checklist creation and readiness from package. | Template state and durable batch/artifact rows remain visible after leaving and returning through backend list endpoints. | Spreadsheet editing, deeper Load Plan export/handoff, direct OTM import, and deeper batch-history analytics remain follow-up scope. |
