...
| `/assets` | OTM-93 | Create asset, upload version, link to module object, download current version, archive asset. | PARTIAL: list/detail/version UI test coverage exists. | TODO. | DONE: assets backend tests cover create, permissions, links, versions. | Archived/versioned state is preserved and visible. | GUI lacks create/upload/link/archive actions. |
...
| `/order-release-generator` | OTM-95 | Select template, create batch, preview XML, generate XML artifact, verify guarded OTM submit state. | PARTIAL: template UI test coverage exists. | TODO. | DONE: OR generator backend tests cover batches, preview, XML artifact, submit guard. | Batch preview/artifact state remains visible. | GUI lacks batch creation and XML artifact actions. |
