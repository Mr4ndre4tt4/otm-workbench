...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `/` and `/home` | OTM-101 | Login, load cockpit, apply active context, edit the context draft without keeping stale submit feedback, refresh readiness, update preferences, navigate away and return. | DONE: `AppFunctionalShell.test.tsx` plus `ContextSwitcher.test.tsx` and `PreferenceControls.test.tsx`; context draft changes clear stale submit feedback and preference clicks keep backend-owned persistence. | DONE: `npm run qa:functional:shell:browser` applies context, edits the domain draft to verify prior submit feedback clears, reapplies context, updates preferences, and checks navigation return-state. | DONE: `test_gui_shell_backend_contracts.py` covers login/session, backend navigation icon/label metadata, active context, persisted preferences, cockpit summary, capabilities, and unauthenticated shell read guards. | Context and preferences remain visible after leaving and returning; draft-level submit feedback does not survive a changed context draft. | None for the current shell foundation journey; future preference work should be explicit new scope. |
