...
| `/master-data` | OTM-114 | Follow the staged Data Factory workflow: select template, validate template, build workbook, upload synthetic workbook, validate relationships, map records, build output, build CSV, export package, register exported package for Load Plan, leave route, and return with backend-owned template state visible. | DONE: `AppFunctionalMasterData.test.tsx` covers staged workflow navigation, template validation, workbook generation, FormData upload, invalid upload recovery, relationship validation, map, output, CSV, export, Load Plan package registration, bearer-auth API assertions, and route return-state. | DONE: `npm run qa:functional:master-data:browser` covers login, Data Factory navigation, explicit `REGIONS_BASIC` selection, template validation, workbook generation, synthetic XLSX upload, relationship validation, map, output, CSV, export package, Load Plan package registration, route return to Output, durable batch/artifact visibility, console health, and HTTP health against local FastAPI + Vite. | DONE: `test_master_data_templates.py` covers template validation, workbook generation, upload parsing, relationship validation, mapping, output records, CSV files, export package, artifacts, evidence, manifest behavior, and idempotent CSV/export retries. `test_load_plan_package_intake.py` covers Master Data package intake, idempotency, evidence, audit, and event creation. | Template state and durable batch/artifact rows remain visible after leaving and returning through backend list endpoints. | Spreadsheet preview/editor, deeper Load Plan checklist handoff, and direct OTM import remain follow-up scope. |
...
/master-data:
  ...
  Load Plan package registration from exported batch, Coordinate Quality
  validate/batches/results/export.
