...
| Integration Mapping Studio | Browser QA passed | Backend is materially stronger after executable preview slices; UI can author and validate rules, the Rules stage has grouped review, join-binding authoring/review is visible, alias-backed mappings can be selected in the UI, and the preview engine executes first-slice qualifier extraction, count-distinct aggregation, explicit multi-hop join-binding provenance, mixed header + loop alias mappings, loop-scoped lookup values, and artifact-level executable JSON assertions for the synthetic NDD-like browser scenario. | To become a full accelerator for NDD-like mappings, it still needs negative alias/path browser recovery (`OTM-153`), richer required-field semantics, response handling, and more transform expression UI. |
...
| `OTM-153` | High | Add negative browser QA for Integration Mapping alias/path recovery, blocked preview messaging, and correction without route reload. |
