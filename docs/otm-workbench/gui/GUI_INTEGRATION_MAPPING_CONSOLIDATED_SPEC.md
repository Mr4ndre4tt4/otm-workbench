...
3. attach source and target payload samples or official schema roots;
4. browse source and target schema paths safely;
5. author mappings, loops, joins, lookups, transforms, and response handling;
6. validate completeness and semantic correctness;
7. generate executable synthetic preview output;
8. generate a technical mapping spec;
9. review artifacts and evidence without exposing real customer data.
```
...
5. Mapping rules must have focused editors, not one long page with every rule
   type.
6. Review, validation, preview, and spec generation must be dedicated steps.
7. Destructive actions such as remove mapping, delete draft, or retire a
   definition must open a dedicated confirmation route or modal with backend
   impact summary.
8. Generated artifacts must use guarded backend downloads only.
9. Suggestions must remain backend-owned and explain confidence/reason.
10. Official OTM paths must come from Catalog/Core schema-pack contracts when
    available; sample payload schemas are complementary, not the only source.
11. No real client payload, CNPJ, CPF, endpoint credential, local file path, or
...
- readiness cards: contract, schemas, mappings, required targets, preview, spec;
- recent artifacts and validation summaries;
- route cards to Systems, Schemas, Mapping, Rules, Review, Preview, Spec,
  Artifacts;
- activity timeline.
```
...
Let the user attach source/target samples or official schema roots and then
inspect schema readiness.
...
3. attach official OTM schema root;
4. search source and target paths;
5. apply backend suggestions to review set;
6. bulk create selected suggestions;
7. create direct mapping manually;
8. create loop and loop-scoped mapping;
9. create multi-hop join binding;
10. create lookup;
11. create response handler;
12. validate and open blocker from review;
13. fix blocker and validate again;
14. preview synthetic target output and inspect provenance;
15. generate spec and download guarded artifact;
16. leave and return to each route without stale state;
17. switch definitions while drafts are dirty;
18. remove mapping and confirm dependent readiness changes;
19. try invalid alias/path and confirm blocked preview;
20. verify no raw customer payload, credential, or local path is exposed.
```
...
5. Payload/schema route with searchable tree/path browser.
6. Mapping workspace with source/target browser and selected-suggestion review.
7. Rule family routes for loops, joins, lookups, response handlers.
8. Dedicated review route.
9. Dedicated preview route with provenance.
10. Dedicated spec/artifacts routes.
11. Browser QA for NDD-like synthetic scenario across routes.
```
...
```text
Find definition -> define contract -> load schemas -> map fields -> author
rules -> validate -> preview -> generate spec -> review artifacts.
```
