...
| OTM Catalog Core | `GUI_CATALOG_CORE_ROADMAP_SPEC.md` | `GUI_CATALOG_CORE_ROUTE_SEPARATION_COMPLETION_REVIEW_2026_05_26.md`, `GUI_CATALOG_CORE_VIEW.md`, `GUI_MODULE_API_CONTRACT_MATRIX.md`, `GUI_MODULE_RECONCILIATION_COMPLETION_REVIEW_OTM86.md`, `output/gui-qa/catalog/` | Route-level redesign spec exists and completion review is recorded. Slice 1A/1B delivered macro-object route recovery and row navigation; Slice 2A delivered table explorer/detail routes; Slice 3A delivered reference options; Slice 4A delivered schema guidance; Slice 5 recorded final QA evidence and follow-up scope. React QA, browser QA, backend QA, build, and screenshots are current. | Move to the next module or create explicit Catalog hardening follow-ups only when prioritized. |
...
| Master Data / Data Factory | `GUI_MASTER_DATA_DATA_FACTORY_REDESIGN_SPEC.md` | `GUI_MASTER_DATA_REDESIGN_COMPLETION_REVIEW_2026_05_26.md`, `GUI_MASTER_DATA_VIEW.md`, `GUI_MASTER_DATA_COMPLETION_REVIEW_OTM115.md`, `GUI_MASTER_DATA_MVP_WORKFLOW_REVIEW_OTM119.md`, `output/gui-qa/master-data/` | Full click-by-click redesign spec exists; Slice 1 hub/route-family separation, Slice 2A template detail, Slice 2B/3 route-level batch execution, Slice 4A list/search/detail, Slice 4B focused create/edit routes, Slice 4C route-level copy polish, Slice 5 Quality Tools separation, and Slice 6 route-recovery QA hardening are delivered with backend, React, browser QA, screenshots, and completion-review evidence. | Update Linear/GitHub evidence from `GUI_MASTER_DATA_REDESIGN_COMPLETION_REVIEW_2026_05_26.md`, then begin `GUI_CATALOG_CORE_ROADMAP_SPEC.md`. |
| Coordinate Quality / Lat-Lon | `GUI_MASTER_DATA_DATA_FACTORY_REDESIGN_SPEC.md` | `GUI_MASTER_DATA_VIEW.md`, `output/gui-qa/master-data/09-quality-tools-hub.png`, `output/gui-qa/master-data/10-lat-lon-validator.png`, `output/gui-qa/master-data/11-lat-lon-batch-detail.png`, `output/gui-qa/master-data/12-lat-lon-export.png` | Delivered as the `Quality Tools` route family under Master Data redesign; Lat/Lon has hub, validator workspace, direct batch detail recovery, backend API tests, React tests, and browser QA evidence. | Create a separate `GUI_COORDINATE_QUALITY_ROADMAP_SPEC.md` only if it becomes a standalone module. |
...
| Developer Tools | `GUI_DEVELOPER_TOOLS_CONSOLIDATED_SPEC.md` | `GUI_BROWSER_RUNTIME_DIAGNOSTIC.md`, `GUI_BROWSER_QA_ATTEMPT.md`, `GUI_MODULE_API_CONTRACT_MATRIX.md`, `output/gui-qa/developer-tools/` | Consolidated objective and route-level controlled technical tools spec exists. Slice 1A replaced the generic placeholder with a dedicated guarded Technical Diagnostics Hub. Slice 2A added backend-owned `/api/v1/platform/dev-tools/summary` with active context, guards, available/disabled tools, disabled reasons, counts, and recent run metadata. Slice 3A added backend-owned Data Dictionary list/detail contracts, an explicit `/dev-tools/data-dictionary` route, URL-owned query state, compact table metadata, Back navigation, and browser evidence. Slice 3B added route-level Data Dictionary table detail UI with compact column metadata and desktop/mobile evidence. Slice 3C added backend-owned FK Catalog relationships, `/dev-tools/fk-catalog`, parent-table links back to Data Dictionary detail, capped visible rows, and desktop/mobile evidence. Slice 3D added backend-owned Schema Pack Diagnostics, `/dev-tools/schema-packs`, Catalog Core schema-pack/root metadata, code-filtered QA seeding, and desktop/mobile evidence. Slice 3E added backend-owned Environment Readiness, `/dev-tools/environment-readiness`, active-context checks, safe environment scope, and desktop/mobile evidence. React, backend, browser QA, and screenshot evidence are current. | Add diagnostic-run detail, FK trace actions, and richer Data Dictionary/FK/schema filtering as explicit next scopes. |
...
1. `GUI_SHELL_AUTH_SIGNOUT_RECOVERY_SPEC.md`
...
2. `GUI_GLOBAL_ACTION_DESTINATION_CONTRACT.md`
...
3. `GUI_ROW_ACCESSIBLE_NAME_CONTRACT.md`
