...
- The Rules stage now includes schema node search and deterministic first-pass
  mapping suggestions with visible confidence and backend reason. The next value
  gap is faceting those suggestions for larger payloads and making suggested
  mapping sets reviewable before creation.
