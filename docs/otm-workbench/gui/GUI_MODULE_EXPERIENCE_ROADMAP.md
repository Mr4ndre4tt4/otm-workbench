...
| Catalog Core | Route separation completion review recorded | Search/validation/macro/schema APIs ready | Route-level catalog explorer | `GUI_CATALOG_CORE_ROADMAP_SPEC.md` captures the route-level Catalog design, and `GUI_CATALOG_CORE_ROUTE_SEPARATION_COMPLETION_REVIEW_2026_05_26.md` records completion evidence. `/catalog/macro-objects/:macroObjectCode`, `/catalog/tables`, `/catalog/tables/:tableName`, `/catalog/reference-options`, and `/catalog/schema-guidance` now carry the deep inspection work while the hub stays compact. Future Catalog work should be explicit hardening or new scope, not completion catch-up. |
...
| Developer Tools | Guarded technical hub, summary contract, Data Dictionary route family, FK Catalog route, Schema Pack Diagnostics route, and Environment Readiness route delivered | Navigation/feature flag, backend summary, safe tool metadata, disabled reasons, recent run metadata, Data Dictionary list/detail, FK Catalog relationship list, schema-pack/root metadata, and active-context readiness metadata exist; remaining diagnostic-run/detail actions are not yet implemented | Controlled technical diagnostics hub | `GUI_DEVELOPER_TOOLS_CONSOLIDATED_SPEC.md` captures the objective and route-level direction. `/dev-tools` now renders a dedicated Technical Diagnostics Hub instead of the generic placeholder, consumes backend-owned `/api/v1/platform/dev-tools/summary`, shows backend gates, available/disabled tools, disabled reasons, and safe recent-run metadata, and exposes explicit route links to `/dev-tools/data-dictionary`, `/dev-tools/fk-catalog`, `/dev-tools/schema-packs`, and `/dev-tools/environment-readiness`. The Data Dictionary Explorer consumes backend-owned metadata, supports URL-owned query state, opens route-level table detail screens, keeps wide table/column metadata compact, and exposes no local file paths. The FK Catalog Explorer consumes backend-owned relationship metadata, keeps the list capped for scanability, and links parent tables back to Data Dictionary detail routes. Schema Pack Diagnostics consumes Catalog Core schema-pack/root metadata, remains read-only, and keeps deep path browsing in Catalog Core. Environment Readiness consumes active-context readiness checks and safe environment scope without exposing connection details or credentials. Keep it hidden from USER and optional for normal delivery. |
...
| Master Data / Data Factory | OTM-115 completion acceptance met for MVP0; 2026-05-26 redesign completion review ready | Backend flows ready for template authoring, backend-owned scenario packs, workbook editor contract/validation/batch creation, filtered/paginated batches, output/CSV preview, CSV/ZIP export, guarded direct OTM import readiness/submit refusal, durable artifacts, Load Plan package registration, cutover checklist creation/readiness with blocker display, Coordinate Quality preview/batches/results/export, date-column CSV alter-session parity, selected-template switch recovery, and hardening QA | Master Data hub -> operational Data Factory -> route-level template detail and batch detail, Template Builder routes, Quality Tools routes | `GUI_MASTER_DATA_DATA_FACTORY_REDESIGN_SPEC.md` remains the implementation source of truth, and `GUI_MASTER_DATA_REDESIGN_COMPLETION_REVIEW_2026_05_26.md` records the completion evidence. Delivered route families now cover `/master-data`, `/master-data/factory`, route-level operational template and batch details, `/master-data/template-builder` list/search/detail/create/edit/copy/guarded-retire routes, and `/master-data/quality` Lat/Lon hub/workspace/batch detail. React QA, browser QA, screenshots, backend tests, and build evidence support the current scope; next GUI planning can move to Catalog Core. |
