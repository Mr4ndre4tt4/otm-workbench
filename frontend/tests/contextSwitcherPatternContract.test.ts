...
describe("GUI context switcher pattern contract", () => {
  it("keeps context-switcher markup centralized in ContextSwitcher", () => {
    const offenders = tsxFilesUnder(sourceRoot)
      .map((path) => ({
        path: sourcePath(path),
        source: readFileSync(path, "utf-8")
      }))
      .filter((file) => file.path !== "app/shell/ContextSwitcher.tsx")
      .filter((file) => file.source.includes("context-switcher"))
      .map((file) => file.path);

    expect(offenders).toEqual([]);
  });

  it("keeps ContextSwitcher exported from the shell barrel", () => {
    const shellBarrel = readFileSync(resolve(sourceRoot, "app/shell/index.ts"), "utf-8");

    expect(shellBarrel).toContain('export { ContextSwitcher } from "./ContextSwitcher"');
  });
});
